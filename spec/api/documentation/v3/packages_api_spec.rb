...
resource 'Packages (Experimental)', type: :api do
  ...
  post '/v3/apps/:guid/packages' do
    ...
    describe 'creating a package' do
      ...
      body_parameter :'data["credentials"]', 'Credentials for private docker image, available fields are user, password, email, login server. ', required: false, parameter_type: 'String'
      body_parameter :'data["store_image"]', 'Whether or not the backend should cache the image. For docker packages only.', default: false,  required: false, valid_values: ['true', 'false'], parameter_type: 'Boolean'
